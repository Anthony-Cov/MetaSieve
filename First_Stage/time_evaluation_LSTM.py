...
def LSTM_tuning(data, lag = 20, layers = 2, bound=400, fwd=5):
...
def forecast_scores(df, levels = 5):
    ...
    for level in range(0,levels):
        ...
        for colname in list_of_columns:
            data = df[colname]
            print(colname, end=' ')
            smape = LSTM_tuning(data, layers = 2 + level,  bound = 400, fwd=5)
